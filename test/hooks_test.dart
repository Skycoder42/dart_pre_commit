...
void main() {
  ...
  group('file task', () {
    ...
    test('returns hasChanges for staged modified files', () async {
      ...
      when(mockFileTask(any)).thenAnswer((_) async => TaskResult.modified);
      final sut = createSut([mockFileTask]);
    ...
    testWithData<Tuple2<bool, int>>('returns rejected on task rejected', const [
      ...
      when(mockFileTask(any)).thenAnswer((_) async => TaskResult.rejected);
      final sut = createSut(
        [mockFileTask],
        fixture.item1,
      );
  ...
  group('repo task', () {
    ...
    test('returns hasChanges for staged modified files and adds them',
      ...
      when(mockRepoTask(any)).thenAnswer((_) async => TaskResult.modified);
      final sut = createSut([mockRepoTask]);
    ...
    testWithData<Tuple3<Iterable<String>?, bool, int>>(
      ...
      when(mockRepoTask(any)).thenAnswer((_) async => TaskResult.rejected);
      final sut = createSut(
